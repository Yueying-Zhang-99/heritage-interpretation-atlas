...
- Research Lens：五种视角对图谱重新着色。多值字段按第一项着色；全部值在详情与聚类中保留。
- Detail：摘要、理论转向、意义、博士关联、笔记、标注、相关节点、来源和 PDF。

**数据说明：用户尚未提供正式 knowledge.json。当前数据库仅采用需求中列出的16个节点，含14个文献条目和2个研究主题，另有6个概念及2个人物辅助节点。摘要、研究编码与关系是示例，需对原文逐条校核后才能作为论文证据。**
...
Timeline 只连接已显示的条目。Network 保留筛选结果及其相邻辅助节点，不会通过连线重新引入被筛掉的文献。未连接的文献仍显示为孤立节点。关系下拉菜单控制连线类型；悬停连线可见证据说明。
...
`Heritage Values` 视角默认按三条理论流着色；其他视角按对应字段首项着色，灰色代表未编码。Cluster 的分组维度与颜色视角独立，方便对比两个编码维度。颜色仅用于分类；不代表发展优劣、重要性或引用量。
