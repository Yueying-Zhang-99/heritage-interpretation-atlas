Atlas.network=function(rows){
  ...
  g.append('path').attr('class','node-dot').attr('d',d=>d3.symbol().type(d.node_type==='person'?d3.symbolDiamond:d.node_type==='concept'?d3.symbolCircle:d3.symbolSquare).size(d.node_type==='document'?180:135)()).attr('fill',d=>d.node_type==='concept'?'#fff':A.color(d)).attr('stroke',A.color).attr('stroke-width',1.5);
