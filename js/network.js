Atlas.network=function(rows){
  ...
  g.append('path').attr('class','node-dot').attr('d',d=>d3.symbol().type(d.node_type==='person'?d3.symbolDiamond:d3.symbolCircle).size(d.node_type==='document'?105+(d.importance||1)*70:135)()).attr('fill',d=>d.placeholder||d.node_type==='concept'?'#fff':A.color(d)).attr('stroke',A.color).attr('stroke-width',1.5).attr('stroke-dasharray',d=>d.placeholder?'3 3':null);
